<template>
    <div class="row">
                        ...
                        <table class="table table-centered table-nowrap">
                            ...
                            <tbody>
                                <tr v-for="(position,index) in positions" v-bind:key="position.id">
                                    ...
                                    <td class="text-center">{{position.rate}}</td>
                                    <td class="text-center">
                                        <a class="mr-3 text-info" data-toggle="tooltip" data-placement="top" title="" data-original-title="View"><i class='bx bx-show'></i></a>
                                        <a class="mr-3 text-warning" @click="edit(position)" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><i class='bx bx-edit-alt' ></i></a>
        ...
        <div class="modal fade exampleModal" id="new" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <admin-position-create @status="message" ref="create"></admin-position-create>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods : {
            ...
            fetch(page_url) {
                ...
                page_url = page_url || this.currentUrl + '/request/admin/positions/'+key;
            ...
            edit(position){
                this.editable = true;
                $("#new").modal('show');
                this.$refs.create.edit(position,true);
            },

            message(val){
                if(val){
                    if(this.editable == true){
                        let page_url = '/request/admin/positions/-?page=' + this.pagination.current_page;
                        this.fetch(page_url);
                    }else{
                        this.fetch();
                    }
