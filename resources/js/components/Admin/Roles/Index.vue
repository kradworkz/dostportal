<template>
    <div class="row">
                            ...
                            <tbody>
                                <tr v-for="(role,index) in roles" v-bind:key="role.id">
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a class="mr-3 text-warning" @click="edit(role)" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><i class='bx bx-edit-alt' ></i></a>
        ...
        <div class="modal fade exampleModal" id="new" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <admin-role-create @status="message" ref="create"></admin-role-create>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods : {
            ...
            fetch(page_url) {
                ...
                page_url = page_url || this.currentUrl + '/request/admin/roles/'+key;
            ...
            edit(role){
                this.editable = true;
                $("#new").modal('show');
                this.$refs.create.edit(role,true);
            },

            message(val){
                if(val){
                    if(this.editable == true){
                        let page_url = '/request/admin/roles/-?page=' + this.pagination.current_page;
                        this.fetch(page_url);
                    }else{
                        this.fetch();
                    }
