<template>
    <div class="row">
                        ...
                        <table class="table table-centered table-nowrap">
                            ...
                            <tbody>
                                <tr v-for="(group,index) in groups" v-bind:key="group.id">
                                    ...
                                    <td class="text-center">{{group.acronym}}</td>
                                    ...
                                    <td class="text-center">
                                        ...
                                        <a class="mr-3 text-warning" @click="edit(group)" data-toggle="tooltip" data-placement="top" title="" data-original-title="Edit"><i class='bx bx-edit-alt' ></i></a>
        ...
        <div class="modal fade exampleModal" id="new" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            <admin-group-create @status="message" ref="create"></admin-group-create>
        </div>
    </div>
</template>
    ...
    export default {
        ...
        methods : {
            ...
            fetch(page_url) {
                ...
                page_url = page_url || this.currentUrl + '/request/admin/groups/'+key;
            ...
            edit(group){
                this.editable = true;
                $("#new").modal('show');
                this.$refs.create.edit(group,true);
            },

            message(val){
                if(val){
                    if(this.editable == true){
                        let page_url = '/request/admin/groups/-?page=' + this.pagination.current_page;
                        this.fetch(page_url);
                    }else{
                        this.fetch();
                    }
